...
export default function ShoppingCartSideBar({ handleSideBar }) {
  ...
  return (
    <SidebarContainer>
      ...
      <ProductListContainer>
        {shoppingCart?.map((product: ShoppingCartProduct, index: number) => (
          <ProductContainer key={index}>
            <ProductPhoto src={product.photo} alt={product.name} />
            <div style={{ maxWidth: "23%" }}>{product.name}</div>
            <div>
              <QuantityText>Qtd:</QuantityText>
              <QuantityContainer>
                <QuantityButton
                  onClick={() => dispatch(fetchQuantityIncrease(product.id))}
                >
                  +
                </QuantityButton>
                <p>{product.quantity}</p>
                <QuantityButton
                  onClick={() => dispatch(fetchQuantityDecrease(product.id))}
                >
                  -
                </QuantityButton>
              </QuantityContainer>
            ...
            <ProductPrice>
              R${Number(product.price).toLocaleString("de-DE")}
            </ProductPrice>
            <RemoveItem onClick={() => dispatch(fetchRemove(product.id))}>
              x
            </RemoveItem>
          </ProductContainer>
        ))}
      ...
      <ProductTotalContainer>
        ...
        <ProductTotal>R${totalPrice?.toLocaleString("de-DE")}</ProductTotal>
      </ProductTotalContainer>
