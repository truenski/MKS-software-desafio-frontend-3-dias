...
function Header() {
  ...
  const addToCart = (product: Product, index: number) => {
    if (shoppingCart && shoppingCart.some((prod) => prod.id === product.id)) {
    ...
    <ProductsContainer>
      {pending ? (
        ...
        productData?.products?.map((product: Product, index: number) => (
          <Container key={index}>
            <ProductPhoto src={product.photo} alt={product.brand} />
            <div style={{ padding: "10px" }}>
              <ContainerPriceName>
                <Name>{product.name}</Name>
                <Price>R${Number(product.price).toLocaleString("de-DE")}</Price>
              </ContainerPriceName>
              <Description>{product.description}</Description>
            </div>
            <BuyButton onClick={() => addToCart(product, index)}>
